...
{
    /* current difficulty formula, fertcoin - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // fertcoin coin: target change every block
    // fertcoin coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);

    if (pindexLast->nHeight+1 >= nHardForkBlock)
    {
        int64_t fertcoinerval = nTargetTimespan / nTargetSpacing;
        bnNew *= ((fertcoinerval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
        bnNew /= ((fertcoinerval + 1) * nTargetSpacing);
    }
    else
    {
        int64_t fertcoinerval = nTargetTimespan / nTargetSpacingOld;
        bnNew *= ((fertcoinerval - 1) * nTargetSpacingOld + nActualSpacing + nActualSpacing);
        bnNew /= ((fertcoinerval + 1) * nTargetSpacingOld);
    }
