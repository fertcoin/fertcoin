...
// Get time weight
int64_t GetWeight(int64_t fertcoinervalBeginning, int64_t fertcoinervalEnd)
{
    return min(fertcoinervalEnd - fertcoinervalBeginning - nStakeMinAge, (int64_t)nStakeMaxAge);
}
...
static int64_t GetStakeModifierSelectiofertcoinervalSection(int nSection)
{
...
static int64_t GetStakeModifierSelectiofertcoinerval()
{
    int64_t nSelectiofertcoinerval = 0;
    for (int nSection = 0; nSection < 64; nSection++) {
        nSelectiofertcoinerval += GetStakeModifierSelectiofertcoinervalSection(nSection);
    }
    return nSelectiofertcoinerval;
}

// select a block from the candidate blocks in vSortedByTimestamp, excluding
// already selected blocks in vSelectedBlocks, and with timestamp up to
// nSelectiofertcoinervalStop.
static bool SelectBlockFromCandidates(
    vector<pair<int64_t, uint256> >& vSortedByTimestamp,
    map<uint256, const CBlockIndex*>& mapSelectedBlocks,
    int64_t nSelectiofertcoinervalStop,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if (fSelected && pindex->GetBlockTime() > nSelectiofertcoinervalStop)
            break;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    int64_t nSelectiofertcoinerval = GetStakeModifierSelectiofertcoinerval();
    int64_t nSelectiofertcoinervalStart = (pindexPrev->GetBlockTime() / getIntervalVersion(fTestNet)) * getIntervalVersion(fTestNet) - nSelectiofertcoinerval;
    const CBlockIndex* pindex = pindexPrev;

    while (pindex && pindex->GetBlockTime() >= nSelectiofertcoinervalStart) {
    ...
    int64_t nSelectiofertcoinervalStop = nSelectiofertcoinervalStart;
    map<uint256, const CBlockIndex*> mapSelectedBlocks;
    for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
        // add an interval section to the current selection round
        nSelectiofertcoinervalStop += GetStakeModifierSelectiofertcoinervalSection(nRound);

        // select a block from the candidates of current round
        if (!SelectBlockFromCandidates(vSortedByTimestamp, mapSelectedBlocks, nSelectiofertcoinervalStop, nStakeModifier, &pindex))
        ...
        if (fDebug || GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                nRound, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectiofertcoinervalStop).c_str(), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    int64_t nStakeModifierSelectiofertcoinerval = GetStakeModifierSelectiofertcoinerval();
    const CBlockIndex* pindex = pindexFrom;
    CBlockIndex* pindexNext = chainActive[pindexFrom->nHeight + 1];

    // loop to find the stake modifier later by a selection interval
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectiofertcoinerval) {
        if (!pindexNext) {
            // Should never happen
